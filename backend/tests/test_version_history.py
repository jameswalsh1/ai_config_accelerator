...
# ---------------------------------------------------------------------------
# Integration: version written during save_config
# ---------------------------------------------------------------------------

class TestVersioningIntegration:
    """Verify that save_config creates a version entry."""

    def test_save_config_creates_version(self, tmp_path: Path):
        from app.services.config_persistence import save_config

        # Create a config file to update
        config_file = tmp_path / "languages" / "test_int.json"
        config_file.parent.mkdir(parents=True)
        config_file.write_text(json.dumps({"id": "test_int", "steps": []}))

        # Save with context so versioning knows scope+target
        save_config(
            config_file,
            {"id": "test_int", "steps": [{"id": "s1"}]},
            validate=False,
            create_backup=False,
        ...
        assert latest["target"] == TARGET

    def test_audit_entry_includes_version(self, tmp_path: Path):
        """The audit JSONL entry should contain a version number."""
        from app.services.config_persistence import save_config
        from app.services.audit_log import LOG_PATH

        config_file = tmp_path / "languages" / "test_aud.json"
        config_file.parent.mkdir(parents=True)
        config_file.write_text(json.dumps({"id": "test_aud"}))
        ...
        assert "version" in last_entry
        assert isinstance(last_entry["version"], int)
