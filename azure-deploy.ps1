...
Write-Host "`n── Ensuring Azure Files share 'mysql-data-2' exists..."
az acr credential show --name $ACR_NAME 2>$null | Out-Null  # warm up az
az.cmd storage share create --name mysql-data-2 --account-name $STORAGE_ACCOUNT --account-key $STORAGE_KEY --output none 2>$null
